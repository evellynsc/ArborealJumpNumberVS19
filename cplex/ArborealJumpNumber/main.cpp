...
int main(int argc, char* argv[]) {
	...
	if (data.contains("input_file_name"))
		auto input_file_name = data["input_file_name"];
	else {
		std::cout << "Nome do arquivo de entrada não encontrado." << std::endl;
		return 1;
	}

	if (data.contains("relaxed"))
	...
	auto input_file = ajns::reader(input_file_name);
